...
import {
  ...
  ReminderButton,
} from "@/components/patterns/client-templates/transactions";
import { TransactionFilterPopover, type TransactionFilterValue } from "@/components/patterns/client-templates/transactions/TransactionFilterPopover";
...
export type PastDueDueSoonTableLiveProps = {
  data: TransactionRow[];
  classOptions: { code: string; label: string }[];
};

export function PastDueDueSoonTableLive({ data, classOptions }: PastDueDueSoonTableLiveProps) {
  const columns = useMemo(() => buildColumns(), []);
  const [filterOpen, setFilterOpen] = useState(false);
  const [filter, setFilter] = useState<TransactionFilterValue>({ classCode: "all", status: "all" });
  ...
  const filteredRows = useMemo(() => {
    return rows.filter((row) => {
      ...
      if (filter.status !== "all" && getTransactionDisplayStatus(row) !== filter.status) return false;
      return true;
    });
  }, [rows, filter]);
