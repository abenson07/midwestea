...
import type { ProgramClassRow } from "@/data/mocks/classes";
...
export type ProgramClassesTableLiveProps = {
  data: ProgramClassRow[];
};

/** Inline "Active Program Classes" card for the real Classes overview. */
export function ProgramClassesTableLive({ data }: ProgramClassesTableLiveProps) {
  const router = useRouter();
  const columns = useMemo(
    () => buildColumns(() => router.push("/admin-migrate/class-detail")),
    [router],
  );
...
/** Full-width "Active classes" view — real data, same grouping/chrome as the demo. */
export function ActiveClassesFullTableLive({ data }: ProgramClassesTableLiveProps) {
  const router = useRouter();
  const columns = useMemo(
    () => buildColumns(() => router.push("/admin-migrate/class-detail")),
    [router],
