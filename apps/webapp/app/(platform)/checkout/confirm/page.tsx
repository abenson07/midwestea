...
function CheckoutConfirmContent() {
  ...
  useEffect(() => {
    ...
    const fetchClassData = async () => {
      try {
        ...
        const url = `/api/classes/by-class-id/${classIDParam}`;
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      const ensureUserResponse = await fetch(`/api/checkout/ensure-user`, {
        method: 'POST',
      ...
      const checkoutResponse = await fetch(`/api/checkout/get-payment-link`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
