...
/**
 * POST /api/enrollments/restore — Restore a soft-removed enrollment (admin only).
 */
export async function POST(request: NextRequest) {
  try {
    ...
    if (updateError) {
      return NextResponse.json(
        { success: false, error: updateError.message || 'Failed to restore enrollment' },
        { status: 500 }
      );
    }
    ...
    return NextResponse.json({
      success: true,
      restoredCount: updatedRows.length,
    });
  } catch (error: unknown) {
    const message = error instanceof Error ? error.message : 'Internal server error';
    console.error('Error in enrollments/restore API:', error);
    return NextResponse.json(
