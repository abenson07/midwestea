...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    try {
      // #region agent log
      debugInfo.push({step:'creating_checkout_session',location:'route.ts:309',data:{classId,stripePriceId:classRecord.stripe_price_id,customerId,successUrl:`${origin}/purchase-confirmation/general`}});
      fetch('http://127.0.0.1:7244/ingest/12521c72-3f93-40b1-89c8-52ae2b633e31',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'create-checkout-session/route.ts:208',message:'Creating Stripe checkout session',data:{classId,stripePriceId:classRecord.stripe_price_id,customerId,successUrl:`${origin}/purchase-confirmation/general`},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'D'})}).catch(()=>{});
      // #endregion
