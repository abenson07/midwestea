...
console.log("[payments/page.tsx] Module loaded");

import { useState, useEffect, Suspense } from "react";
import { useRouter, useSearchParams } from "next/navigation";
import { DataTable } from "@/components/ui/DataTable";
import { DetailSidebar } from "@/components/ui/DetailSidebar";
import { getTransactions, type TransactionWithDetails } from "@/lib/payments";
import { formatCurrency } from "@midwestea/utils";

function TransactionsPageContent() {
    console.log("[TransactionsPageContent] Component rendering");
    ...
    useEffect(() => {
        console.log("[TransactionsPageContent] useEffect running, calling loadTransactions");
        loadTransactions();
    }, []);
    ...
    const loadTransactions = async () => {
        ...
        try {
            ...
            console.log("Transactions response:", { fetchedTransactions, fetchError });
            if (fetchError) {
                console.error("Error loading transactions:", fetchError);
                setError(fetchError);
                setTransactions([]);
            } else if (fetchedTransactions) {
                console.log("Transformed transactions:", fetchedTransactions);
    ...
    const handleRowClick = (transaction: TransactionWithDetails) => {
        router.push(`/payments?transactionId=${transaction.id}`);
        setSelectedTransaction(transaction);
        setIsSidebarOpen(true);
    };
